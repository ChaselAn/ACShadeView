...
extension UIView {
  ...
  func ac_shadeView(withColor color: UIColor, fromAlpha: CGFloat = 0, toAlpha: CGFloat = 1, direction: ACShadeDirection = .topToBottom) {
  ...
  /// - parameter ToAlpha:   到什么透明度结束
  /// - parameter direction: 渐变的方向
  func ac_shadeView(WithRGBcolor color: String, fromAlpha: CGFloat = 0, toAlpha: CGFloat = 1, direction: ACShadeDirection = .topToBottom) {
  ...
  /// - parameter direction: 渐变的方向
  func ac_shadeView(withColorList colorList: [UIColor], direction: ACShadeDirection = .topToBottom) {
    let gradientLayer = CAGradientLayer()
  ...
  /// - parameter direction: 渐变的方向
  func ac_shadeView(withRGBcolorList RGBcolorList: [String], direction: ACShadeDirection = .topToBottom) {
    ac_shadeView(withColorList: RGBcolorList.map({ ac_hexStringToColor($0) }), direction: direction)
  }
  
  // 根据方向计算起点与终点
  fileprivate func ac_startAndEndPoint(_ gradientLayer: CAGradientLayer,direction: ACShadeDirection){
